...
#
# layer-225 branch: negotiate the PUBLISHED layer 225; parsers are
# generated from tools/schema_l225.tl (the true 225 dialect — the
# published 223 schema plus the layer 224/225 dumps from
# core.telegram.org/api/layers; assemble with target/build_l225.py).
# Canonical ids ARE the wire ids for the negotiated layer. Aliases are
# the exception, kept only when justified:
#   - a 223-era id whose shape is layout-compatible with the 225 ctor
#     (old payloads decode correctly through the new shape);
#   - a draft id production DCs answer with regardless of the
#     negotiated layer (channel — wire-verified).
# A re-issued id whose shape is NOT compatible is deliberately absent:
# decoding an old payload through the new shape would corrupt fields,
# so an unknown-constructor error is preferable. (Computed by
# target/prune_aliases.py against tl.json / schema.tl / schema_l225.tl.)
CTOR_ALIASES = {
    # shape-compatible 223-era ids (a re-issued ctor's old id; the 225
    # shape only adds flag-gated fields, so old payloads still decode):
    "InputMedia": {
        0xb3ba0635: "inputMediaPhoto",  # 223 id
    },
    "MessageMedia": {
        0x695150d7: "messageMediaPhoto",  # 223 id
    },
    "ReactionsNotifySettings": {
        0x56e34970: "reactionsNotifySettings",  # 223 id
    },
    "Update": {
        0xaca1657b: "updateMessagePoll",  # 223 id
    },
    "UserFull": {
        0xa02bc13e: "userFull",  # 223 id
    },
    # draft ids production DCs answer with regardless of the negotiated
    # layer (wire-verified: channel arrived as the tdlib-draft id at a
    # negotiated 223). The draft shape carries flag-gated fields the
    # 225 parser does not know; a payload setting them fails loudly
    # rather than desyncing.
    "Chat": {
        0xd49f34c6: "channel",  # 229 draft id
    },
}
