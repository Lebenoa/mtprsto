...
use crate::types::{
    ...
    CHANNELS_DELETE_CHANNEL, CHANNELS_DELETE_MESSAGES, CHANNELS_EDIT_ADMIN, CHANNELS_GET_CHANNELS,
    CHANNELS_GET_MESSAGES, CHANNELS_GET_PARTICIPANTS, CHANNELS_INVITE_TO_CHANNEL,
    CHANNELS_JOIN_CHANNEL, CHANNELS_LEAVE_CHANNEL, CONTACTS_RESOLVE_PHONE,
    CONTACTS_RESOLVE_USERNAME, CONTACTS_SEARCH, DOCUMENT_ATTRIBUTE_FILENAME, Dialog, Dialogs,
    FileLocation, HELP_GET_CONFIG, HELP_GET_NEAREST_DC, INPUT_MEDIA_CONTACT, INPUT_MEDIA_EMPTY,
    INPUT_MEDIA_GEO_POINT, INPUT_MEDIA_UPLOADED_DOCUMENT, INPUT_MEDIA_UPLOADED_PHOTO,
    INPUT_MESSAGE_ID, INPUT_PHOTO, INPUT_REPLY_TO_MESSAGE, INPUT_SINGLE_MEDIA,
    INPUT_WEB_FILE_LOCATION, InputChannel, InputPeer, InputUser, MESSAGES_DELETE_HISTORY,
    MESSAGES_DELETE_MESSAGES, MESSAGES_DIALOGS, MESSAGES_DIALOGS_SLICE, MESSAGES_EDIT_CHAT_ABOUT,
    MESSAGES_EDIT_MESSAGE, MESSAGES_FORWARD_MESSAGES, MESSAGES_GET_BOT_CALLBACK_ANSWER,
    MESSAGES_GET_DIALOGS, MESSAGES_GET_HISTORY, MESSAGES_GET_MESSAGES, MESSAGES_IMPORT_CHAT_INVITE,
...
/// Build `channels.deleteChannel` payload — deactivates the channel,
/// which is the only exit for its creator (leaveChannel answers
/// `USER_NOT_PARTICIPANT` for them).
#[must_use]
pub fn build_delete_channel(channel: &InputChannel) -> Vec<u8> {
    let mut w = TLWriter::new();
    w.write_u32(CHANNELS_DELETE_CHANNEL);
    channel.write_to(&mut w);
    w.into_bytes()
}
